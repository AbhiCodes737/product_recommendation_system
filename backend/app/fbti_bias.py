...
def count_items_bias(pid, points):
    item_counts = {}
    ...
    for key, value in item_counts.items():
        ...
        result.append({"pid": key, "similarity_score": value, "discounted_price": discounted_price, "bias": bias})
    return result

def count_all_bias(pid, similar):
    ...
    if gender == 'M':
        ...
        notallowed = 'M'
    total_simi_items = count_items_bias(pid, 1)
    combined_items = {}
    
    for item in total_simi_items:
        if item['pid'] not in combined_items:
            combined_items[item['pid']] = {"pid": item['pid'], "similarity_score": item['similarity_score'], "discounted_price": item['discounted_price'], "bias": item["bias"]}
        else:
    ...
    if len(combined_items) < 5:
        for item in similar:
            ...
            similar_items = count_items_bias(similar_pid, 0.5)
    ...
    for item in final_list:
        if item['pid'] not in seen_pids and item['pid'] not in [s['pid'] for s in similar] and item['pid'] != pid and item['bias'] != notallowed:
            overall.append(item)
...
def generate_similar_bias(pid):
    similar = find_nearest_neighbors(pid)
    return jsonify(count_all_bias(pid, similar))
